...
class FireBuildingModel(object):
    def __init__(self, name: str, initial_fire_level=16):
        ...
        self.extinguished_state.handlers = {"enter": self.extinguished_enter}
        ...
        self.sm.add_transition(
            self.idle_state, self.on_fire_state, events=["ignition_event"]
        )
        self.sm.add_transition(
            self.extinguished_state, self.on_fire_state, events=["ignition_event"]
        )

        self.sm.add_transition(
            self.on_fire_state, self.idle_state, events=["reset_event"]
        )
        self.sm.add_transition(
            self.extinguished_state, self.idle_state, events=["reset_event"]
        )

        self.sm.add_transition(
            self.on_fire_state,
            self.extinguished_state,
            events=["fire_extinguished_event"],
        )
    ...
    def dispatch(self, event):
        ...
        if new_state != prev_state:
            logger.debug(
                f"BUILDING {self.name}: State changed to {new_state} from {prev_state} on {event.name}"
            )
    ...
    def fire_doused_action(self, state, event):
        if (
            self.sm.state == self.on_fire_state
...
class HeaterBuildingModel(object):
    ...
    def on_fire_enter(self, state, event):
        ...
        self.heater_timer.function = self.on_fire_timeout
        self.heater_timer.set_timeout(self.on_fire_duration)
        self.heater_timer.start()

    def idle_enter(self, state, event):
        self.heater_timer.reset()

    def on_fire_timeout(self):
        self.reset()
